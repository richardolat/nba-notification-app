...
def process_game_data(data):
    messages = []
    for game in data:
        # Only process games that have scores
        if game['HomeTeamScore'] is not None and game['AwayTeamScore'] is not None:
            messages.append(f"{game['HomeTeam']} vs {game['AwayTeam']} | Final Score: {game['HomeTeamScore']} - {game['AwayTeamScore']}")
        else:
            messages.append(f"{game['HomeTeam']} vs {game['AwayTeam']} | Status: {game['Status']}")
    return "\n".join(messages)
